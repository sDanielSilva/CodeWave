      ...
      <nav class="navbar align-items-start sidebar sidebar-dark accordion bg-gradient-primary p-0 navbar-dark">
        <div class="container-fluid d-flex flex-column p-0">
          ...
          <?php
              ...
              <li class="nav-item"><a class="nav-link active" href="zonas.php"><i class="fas fa-map-marker-alt"></i><span>Zonas</span></a></li>
                      ...
                      <div class="row align-items-center no-gutters">
                        <div class="col me-2">
                          ...
                          <div class="text-dark fw-bold h5 mb-0">
                            <span> <?php require "./zonas/totalZona.php"; ?> </span>
                          </div>
                        </div>
                        <div class="col-auto">
                ...
                <div class="modal-dialog modal-dialog-centered">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title">Adicionar Zona</h5>
                      <button type="button" class="btn-close" data-bs-dismiss="modal" id="btnFechar" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                      <form id="formAdicionarZona" method="POST" action="./zonas/adicionarZona.php">
                        <div class="mb-3">
                          <label for="nomeZona" class="form-label">Nome da Zona</label>
                          <input type="text" class="form-control" id="nomeZona" name="nomeZona">
                        </div>
              ...
              <div id="modalEditar" class="modal" style="display: none;">
                <div class="modal-dialog modal-dialog-centered">
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      <form id="formEditarZona" method="POST" action="./zonas/editarZona.php">
                        <input type="hidden" id="edit_id_zona" name="edit_id_zona">
                        <div class="mb-3">
                          <label for="edit_nomeZona" class="form-label">Novo Nome da Zona</label>
                          <input type="text" class="form-control" id="edit_nomeZona" name="edit_nomeZona">
                        </div>
              ...
              <div class="container-fluid">
                <div class="card shadow">
                  <div class="card-header py-3">
                    <p class="text-primary m-0 fw-bold">Zona Informações</p>
                  </div>
                  <div class="card-body">
...
<script>
    $(document).ready(function() {
        var pagina_atual = 1;
        var zonas_por_pagina = 7;
        var zonas_cache = [];

        function carregarZonas(pagina) {
            pagina_atual = pagina; // Atualiza a página atual

            // Verifica se os dados já estão em cache
            if (zonas_cache[pagina]) {
                renderizarZonas(zonas_cache[pagina]);
            } else {
                $.ajax({
                    url: './zonas/zonasScript.php?pagina=' + pagina,
                    type: 'get',
                    dataType: 'json',
                    success: function(data) {
                        zonas_cache[pagina] = data; // Armazena os dados em cache
                        renderizarZonas(data);
                    }
                });
            }
        }

        function renderizarZonas(data) {
            $('#dataTable tbody').empty();
            $.each(data.zonas, function(key, row) {
                var tr = $('<tr>');
                tr.append('<td>' + row.id_zona + '</td>');
                tr.append('<td>' + row.nome + '</td>');
                tr.append('<td><button class="btn btn-warning text-white btn-sm" onclick="editarZona(' + row.id_zona + ', \'' + row.nome + '\')">Editar</button><button class="btn btn-danger btn-sm" onclick="apagarZona(' + row.id_zona + ')">Apagar</button></td>');
                $('#dataTable tbody').append(tr);
            });

            var total_paginas = Math.ceil(data.total / zonas_por_pagina);
        ...
        carregarZonas(pagina_atual);
                    ...
                    <div id="confirm-delete-modal" class="modal" style="display: none;">
                      <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                          <div class="modal-header">
                            <h5 class="modal-title">Excluir Zona</h5>
                          </div>
                          <div class="modal-body">
                            <p>Tem certeza que deseja apagar esta zona?</p>
                          </div>
                          <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Não</button>
                            <button type="button" class="btn btn-danger" onclick="deleteZona(<?php echo $id_zona; ?>)">Sim </button>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <script>
                      ...
                      document.addEventListener("DOMContentLoaded", function() {
                        ...
                        function editarZona(idZona, nomeZona) {
                        document.getElementById("modalEditar").style.display = "block";
                        document.getElementById("overlay").style.display = "block";
                        document.getElementById("edit_id_zona").value = idZona;
                        document.getElementById("edit_nomeZona").value = nomeZona;
                      }
                      document.addEventListener("DOMContentLoaded", function() {
                        var nomeZonaInput = document.getElementById("nomeZona");
                        nomeZonaInput.addEventListener("input", function() {
                          if (this.value.trim() !== "") {
                            this.classList.remove("is-invalid");
                          }
                        });
                        document.getElementById("formAdicionarZona").addEventListener("submit", function(event) {
                          var nomeZona = nomeZonaInput.value.trim();
                          if (nomeZona === "") {
                            event.preventDefault();
                            nomeZonaInput.classList.add("is-invalid");
                          }
                        });
                        document.getElementById("btnFechar").addEventListener("click", function() {
                            nomeZonaInput.classList.remove("is-invalid");
                        });
                      });
                      ...
                      function apagarZona(idZona) {
                        ...
                        document.getElementById('confirm-delete-modal').querySelector('.btn-danger').addEventListener('click', function() {
                          window.location.href = './zonas/deleteZona.php?id_zona=' + idZona;
                        });
