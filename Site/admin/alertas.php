      ...
      <div class="container-fluid d-flex flex-column p-0">
        ...
        <ul class="navbar-nav text-light" id="accordionSidebar">
          ...
          <li class="nav-item"><a class="nav-link" href="eventos.php"><i
                class="fas fa-calendar"></i><span>Eventos</span></a></li>
          <li class="nav-item"><a class="nav-link" href="zonas.php"><i
                class="fas fa-map-marker-alt"></i><span>Zonas</span></a></li>
          <li class="nav-item"><a class="nav-link" href="produtos.php"><i
                class="fas fa-box"></i><span>Produtos</span></a></li>
          <li class="nav-item"><a class="nav-link" href="funcionarios.php"><i
                class="fas fa-address-book"></i><span>Funcionários</span></a></li>
          <li class="nav-item"><a class="nav-link" href="cargo.php"><i
                class="fas fa-address-card"></i><span>Cargos</span></a></li>
          <li class="nav-item"><a class="nav-link" href="feedback.php"><i
                class="fas fa-quote-right"></i><span>Feedbacks</span></a></li>
          ...
          <li class="nav-item"><a class="nav-link" href="gerir_horario.php"><i
                class="fas fa-calendar"></i><span>Horários</span></a></li>
          <li class="nav-item"><a class="nav-link" href="noticias.php"><i
                class="fas fa-newspaper"></i><span>Notícias</span></a></li>
          <li class="nav-item"><a class="nav-link" href="progressao.php"><i
                class="fas fa-id-card"></i><span>Progressão</span></a></li>
                    ...
                    <div class="row align-items-center no-gutters">
                      <div class="col me-2">
                        <div class="text-uppercase text-primary fw-bold text-xs mb-1">
                          <span>Número de Alertas</span>
                        </div>
                        <div class="text-dark fw-bold h5 mb-0">
                          <span> <?php require "./alertas/totalAlerta.php"; ?> </span>
                        </div>
                      </div>
                      <div class="col-auto">
                        <i class="fas fa-danger fa-2x text-gray-300">!</i>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="container-fluid">
                <div class="card shadow">
                  <div class="card-header py-3">
                    <p class="text-primary m-0 fw-bold">Alertas Informações</p>
                  </div>
                  <div class="card-body">
                    ...
                    <script>
                      $(document).ready(function () {
                        var pagina_atual = 1;
                        var alertas_por_pagina = 10; // Define o número de alertas por página
                        var alertas_cache = [];

                        function carregarAlertas(pagina) {
                          pagina_atual = pagina; // Atualiza a página atual

                          // Verifica se os dados já estão em cache
                          if (alertas_cache[pagina]) {
                            renderizarAlertas(alertas_cache[pagina]);
                          } else {
                            $.ajax({
                              url: './alertas/alertaScript.php?pagina=' + pagina,
                              type: 'get',
                              dataType: 'json',
                              success: function (data) {
                                alertas_cache[pagina] = data; // Armazena os dados em cache
                        ...
                        function renderizarAlertas(data) {
                          ...
                          $.each(data.alertas, function (key, row) {
                            var tr = $('<tr>');
                            tr.append('<td>' + row.id_alerta + '</td>');
                            tr.append('<td>' + row.data + '</td>');
                            tr.append('<td>' + row.mensagem + '</td>');
                            $('#dataTable tbody').append(tr);
                          });

                          var total_paginas = Math.ceil(data.total / alertas_por_pagina);
                          ...
                          $('.pagination .prev').off('click').on('click', function (e) {
                            e.preventDefault();
                            if (pagina_atual > 1) {
                              carregarAlertas(pagina_atual - 1);
                            }
                          });
