    ...
    <nav class="navbar align-items-start sidebar sidebar-dark accordion bg-gradient-primary p-0 navbar-dark">
      <div class="container-fluid d-flex flex-column p-0">
        ...
        <?php
              ...
              <li class="nav-item"><a class="nav-link active" href="funcionarios.php"><i class="fas fa-address-book"></i><span>Funcionários</span></a></li>
          ...
          <div class="row">
                    ...
                    <div class="row align-items-center no-gutters">
                      <div class="col me-2">
                        ...
                        <div class="text-dark fw-bold h5 mb-0">
                          <span> <?php require "./funcionarios/totalFuncionarios.php"; ?> </span>
                        </div>
                      </div>
                      <div class="col-auto">
                  ...
                  <div class="modal-body">
                    <form id="formAdicionarFuncionario" method="POST" action="./funcionarios/adicionarFuncionario.php">
                      ...
                      <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="passwordFuncionario" name="passwordFuncionario">
                      </div>
                      <div class="mb-3">
                      ...
                      <div class="mb-3">
                        ...
                        <select id="cargo_funcionario" name="cargo_funcionario" class="form-select"></select>
                      </div>
            ...
            <script>
              $(document).ready(function () {
                $.ajax({
                  url: './funcionarios/getCargos.php',
                  type: 'GET',
                  dataType: 'json',
                  success: function (data) {
                    var $select = $('#cargo_funcionario');
                    $select.empty();
                ...
                $('#cargo_funcionario').change(function () {
                  var selectedId = $(this).val();
                  console.log('Cargo selecionado ID:', selectedId);
                });
              });
            </script>
              ...
              <div class="card shadow">
                ...
                <div class="card-body">
                  ...
                  <script>
                    $(document).ready(function () {
                      var pagina_atual = 1;
                      var funcionarios_por_pagina = 7;
                      var funcionarios_cache = [];

                      function carregarFuncionarios(pagina) {
                        pagina_atual = pagina; // Atualiza a página atual

                        // Verifica se os dados já estão em cache
                        if (funcionarios_cache[pagina]) {
                          renderizarFuncionarios(funcionarios_cache[pagina]);
                        } else {
                          $.ajax({
                            url: './funcionarios/funcionariosScript.php?pagina=' + pagina,
                            type: 'get',
                            dataType: 'json',
                            success: function (data) {
                              funcionarios_cache[pagina] = data; // Armazena os dados em cache
                      ...
                      function renderizarFuncionarios(data) {
                        ...
                        $.each(data.funcionarios, function (key, row) {
                          var tr = $('<tr>');
                          tr.append('<td>' + row.id_funcionario + '</td>');
                          tr.append('<td><img src="' + row.img + '" width="50px" height="50px" style="border-radius: 50%;"></td>');
                          tr.append('<td>' + row.nome + '</td>');
                          tr.append('<td><div class="input-group"><input type="password" class="form-control password-field" value="' + row.password + '" disabled style="width: 30px;"><div class="input-group-append"><button class="btn btn-outline-secondary toggle-password" type="button"><i class="fas fa-eye-slash"></i></button></div></div></td>');
                          tr.append('<td>' + row.email + '</td>');
                          tr.append('<td><button class="btn btn-warning text-white btn-sm" onclick="editarFuncionario(' + row.id_funcionario + ', \'' + row.nome + '\', \'' + row.password + '\', \'' + row.email + '\', \'' + row.img + '\')">Editar</button><button class="btn btn-danger btn-sm" onclick="apagarFuncionario(' + row.id_funcionario + ')">Apagar</button></td>');
                          $('#dataTable tbody').append(tr);
                        });

                        var total_paginas = Math.ceil(data.total / funcionarios_por_pagina);
                        ...
                        $('.pagination .prev').off('click').on('click', function (e) {
                          e.preventDefault();
                          if (pagina_atual > 1) {
                            carregarFuncionarios(pagina_atual - 1);
                          }
                        });
                  ...
                  <div id="confirm-delete-modal" class="modal" style="display: none;">
                    <div class="modal-dialog modal-dialog-centered">
                      <div class="modal-content">
                        ...
                        <div class="modal-body">
                          <p>Tem certeza que deseja apagar este funcionário?</p>
                        </div>
                        <div class="modal-footer">
                          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Não</button>
                          <button type="button" class="btn btn-danger" onclick="deleteZona(
                                                                        <?php echo $id_funcionario; ?>)">Sim </button>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <script>
                    document.addEventListener("DOMContentLoaded", function () {
                      ...
                      document.getElementById("formAdicionarFuncionario").addEventListener("submit", async function (event) {
                        ...
                        if (imagemFuncionarioInput.files.length > 0) {
                          ...
                          try {
                            var response = await fetch('https://api.imgbb.com/1/upload?key=<?php echo getenv("IMGBB_API_KEY"); ?>', {
                              method: 'POST',
                              body: formData
                            });
                    ...
                    function apagarFuncionario(idFuncionario) {
                      showDeleteConfirmationPopup();
                      document.getElementById('confirm-delete-modal').querySelector('.btn-danger').addEventListener('click', function () {
                        window.location.href = './funcionarios/deleteFuncionario.php?id_funcionario=' + idFuncionario;
                      });
